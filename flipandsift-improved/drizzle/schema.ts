...
/**
 * Domain Feed table — stores discovered expired/dropping domains
 * Replaces dependency on SpamZilla with our own domain discovery engine
 */
export const domainFeed = mysqlTable("domainFeed", {
  id: int("id").autoincrement().primaryKey(),
  domain: varchar("domain", { length: 255 }).notNull().unique(),
  tld: varchar("tld", { length: 20 }).notNull(),
  
  // Domain status
  status: mysqlEnum("status", ["pending_delete", "auction", "available", "dropped", "registered"]).default("pending_delete").notNull(),
  
  // Important dates
  dropDate: timestamp("dropDate"), // When domain will drop (pending_delete)
  auctionEndDate: timestamp("auctionEndDate"), // When auction ends (auction)
  
  // Pricing
  currentPrice: int("currentPrice"), // Current bid/price in cents
  buyNowPrice: int("buyNowPrice"), // Buy now price in cents (if applicable)
  
  // Source tracking
  source: varchar("source", { length: 100 }).notNull(), // e.g., 'expireddomains.net', 'godaddy_auctions', 'registry'
  sourceUrl: varchar("sourceUrl", { length: 500 }), // Link to original listing
  
  // SEO Metrics (JSON for flexibility)
  metrics: text("metrics"), // JSON string: { trustFlow, citationFlow, referringDomains, domainAge, archiveOrgSnapshots }
  
  // Opportunity score (calculated)
  opportunityScore: int("opportunityScore"), // 0-100, calculated by our algorithm
  
  // Niche/topic classification
  niche: varchar("niche", { length: 100 }), // Auto-detected niche (health, tech, finance, etc.)
  
  // Processing status
  isEnriched: int("isEnriched").default(0).notNull(), // 0 = basic data, 1 = full metrics collected
  enrichedAt: timestamp("enrichedAt"),
...
export type DomainFeed = typeof domainFeed.$inferSelect;
export type InsertDomainFeed = typeof domainFeed.$inferInsert;
